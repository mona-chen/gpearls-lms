...
class LmsQuiz < ApplicationRecord
  ...
  # Attribute accessors for compatibility
  attr_accessor :published

  # Associations
  belongs_to :course, class_name: "Course", optional: false
  belongs_to :chapter, class_name: "CourseChapter", optional: true
  belongs_to :creator, class_name: "User", foreign_key: :creator, optional: true
  belongs_to :created_by, class_name: "User", optional: true
  belongs_to :updated_by, class_name: "User", optional: true

  has_many :lms_quiz_questions, foreign_key: :quiz_id, dependent: :destroy
  has_many :lms_questions, through: :lms_quiz_questions
  ...
  def published=(value)
    @published = value
    self.status = value ? "Published" : "Draft"
  end
  ...
  def total_marks
    lms_quiz_questions.sum(:marks)
  end

  def passing_marks
    (total_marks * passing_percentage / 100).round(2)
  end
