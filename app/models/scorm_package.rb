class ScormPackage < ApplicationRecord
  ...
  has_many :scorm_completions, dependent: :destroy
  ...
  attribute :manifest_content, :text
  attribute :extracted_path, :string
  attribute :metadata, :json
  attribute :error_message, :text

   enum :status, [ :uploaded, :extracting, :extracted, :error ]
  ...
  def extract_package
    ...
    begin
      ...
      package_files.each do |file|
        if file.content_type == "application/zip" || file.filename.to_s.end_with?(".zip")
          extract_zip_file(file, extraction_path)
        else
          # Handle individual files
          file_path = extraction_path.join(file.filename.to_s)
      ...
      if manifest_data
        update!(
          ...
          launch_file: manifest_data[:launch_file] || "index.html",
          version: manifest_data[:version] || "SCORM 2004",
          extracted_path: extraction_path.to_s,
          metadata: manifest_data[:metadata] || {}
        )
        true
      else
        update!(status: :error, error_message: "No valid SCORM manifest found")
        false
      end
    rescue => e
  ...
  def extraction_duration
    # Simulate extraction time
    30
  end

  def contains_malicious_content?(content)
    # Basic security checks for SCORM content
    malicious_patterns = [
      /<script[^>]*>.*?<\/script>/mi,
      /javascript:/i,
      /vbscript:/i,
    ...
    malicious_patterns.any? { |pattern| content =~ pattern }
  end



  def detect_scorm_version(manifest_content)
    if manifest_content.include?("2004 3rd Edition") || manifest_content.include?("2004")
      self.version = "SCORM 2004"
    elsif manifest_content.include?("1.2")
      self.version = "SCORM 1.2"
    else
  ...
  def self.create_from_upload(uploaded_file, lesson, user)
    package = create!(
      course_lesson: lesson,
      uploaded_by: user,
      title: File.basename(uploaded_file.try(:original_filename) || "course", ".*"),
    ...
    # Only attach if it's a real uploaded file (not a test double)
    package.package_files.attach(uploaded_file) if uploaded_file.respond_to?(:tempfile) || uploaded_file.is_a?(ActionDispatch::Http::UploadedFile)
    package
  end
