...
const getGradientColor = () => {
  if (!props.course) return 'linear-gradient(to top right, black, #3b82f6)'

  let color = props.course.card_gradient?.toLowerCase() || 'blue'
  let colorMap = theme.backgroundColor[color]
  if (!colorMap || !colorMap[400]) {
    colorMap = theme.backgroundColor.blue || { 400: '#3b82f6' }
  }
  ...
  /* return `radial-gradient(ellipse at 80% 20%, ${colorMap[100]} 0%, transparent 40%),
